...
class Main {
	...
	private static int find(int i, int i2) {
		...
		if (Dp[i][i2]==NULL) {
			...
			} else ans=Math.min(1+find(i+1,i2),1+find(i,i2-1));
			Dp[i][i2]=ans;
		}
		return Dp[i][i2];
	}

	private static void trace(int i, int i2, StringBuilder sb) {
		if (i>i2) return;
		if (Text.charAt(i)==Text.charAt(i2)) {
			sb.append(Text.charAt(i));
			trace(i+1,i2-1,sb);
			if (i!=i2) sb.append(Text.charAt(i2));
		} else if (find(i,i2)==1+find(i+1,i2)) {
			sb.append(Text.charAt(i));
			trace(i+1,i2,sb);
			sb.append(Text.charAt(i));
		} else if (find(i,i2)==1+find(i,i2-1)) {
