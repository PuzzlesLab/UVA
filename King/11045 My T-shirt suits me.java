...
class Main {
	...
	public static void main(String [] args) throws Exception {
		...
		final int T=6;
		final HashMap<String,Integer> sizeMap=new HashMap<>();
		sizeMap.put("XXL",0);
		sizeMap.put("XL",1);
		sizeMap.put("L",2);
		sizeMap.put("M",3);
		...
		for (int tc=0;tc<TC;tc++) {
			...
			int tAmount=Integer.parseInt(st.nextToken())/T;
			int M=Integer.parseInt(st.nextToken());
			int sNode=T+M;
			int tNode=sNode+1;
			int maxNode=tNode+1;
			NodeMap=new Node[maxNode];
			EdgeMap=new Edge[maxNode][maxNode];
			for (int i=0;i<maxNode;i++) NodeMap[i]=new Node();

			// Create virtual start node <-> size.
			for (int t=0;t<T;t++) addEdge(sNode,t,tAmount);
			// Create volunteer <-> virtual end node.
			for (int m=T;m<sNode;m++) addEdge(m,tNode,1);
			// Create size <-> volunteer
			for (int m=T;m<sNode;m++) {
				st=new StringTokenizer(br.readLine());
				while (st.hasMoreTokens()) addEdge(sizeMap.get(st.nextToken()),m,1);
			}
